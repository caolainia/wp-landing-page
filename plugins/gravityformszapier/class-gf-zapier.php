...
class GF_Zapier extends GFFeedAddOn {
	...
	/***
	 * Registers the sample entry REST endpoint.
	 *
	 * @since 4.0
	 */
	public function register_rest_routes() {

		// Load requirements endpoint. This is used by the Zapier app to ensure that the requirements are met.
		require_once( plugin_dir_path( __FILE__ ).'includes/class-controller-requirements.php' );
		$controller = new GF_REST_Requirements_Controller();
		$controller->register_routes();

		// Loading sample entry endpoint.
		if ( class_exists( 'GFFeedAddOn' ) ) {
			require_once( plugin_dir_path( __FILE__ ).'includes/class-controller-sample-entry.php' );
			$controller = new GF_REST_Sample_Entry_Controller();
			$controller->register_routes();
		}
	}
	...
	/**
	 * Retain original settings values, with a few exceptions.
	 *
	 * @since 4.0
	 *
	 ...
	 *
	 * @return bool If feed settings were saved.
	 */
	public function save_feed_settings( $feed_id, $form_id, $settings ) {
		// Return early if editing a legacy feed.
		if ( $this->is_legacy_feed( $this->get_feed( $feed_id ) ) ) {
		...
		foreach ( $settings as $setting => $value ) {
			if ( ! in_array( $setting, $allowed, true ) ) {
				...
				$original_value = rgars( $this->get_current_feed(), 'meta/' . $setting );
				if ( null !== $original_value ) {
					$settings[ $setting ] = $original_value;
				}
			}
		}
	...
	/**
	 * Override function in framework so the add new button can be "disabled" and a message provided.
	 *
	 * Also display toggle to Show/Hide Legacy Feeds.
	 *
	 * @since 4.0
	 */
	public function feed_list_title() {
		...
		$input_text = sprintf(
			'<input class="add-new-h2" type="button" value="%s" onclick="jQuery( \'#feed_message\' ).slideDown()">',
			esc_html__( 'Add New', 'gravityforms' )
		);

		$message = '<div id="feed_message" style="display:none;"><div class="alert_yellow" style="height:50px;"><p style="padding:15px;">' . $this->feed_list_no_item_message() . '</p></div></div>';

		return $this->form_settings_title() . $input_text . $message;
	}
	...
	private function should_display_feeds() {
		...
		$state = $this->get_plugin_settings( 'display_feeds' );
	...
	public function feed_list_no_item_message() {
		return sprintf(
			...
			esc_html__( 'Simply %1$screate a zap%2$s on zapier.com. %3$sLearn more.%4$s', 'gravityformszapier' ),
